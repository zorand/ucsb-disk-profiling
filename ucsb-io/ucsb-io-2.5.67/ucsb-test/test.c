...
main(int argc, char **argv){
	...
	ret = posix_memalign(&buf, 4096, 100*4096);
	if (ret) {
		printf("Error in memalign\n");
		exit(-1);
	}
	printf("%d %p\n",ret,buf);
	...
	printf("QoS: type=%d, prior=%d, rate=%d\n",qos.type,qos.priority,qos.rate);

	while ((ret=read(fd,buf,10*4096)) > 0){
		printf("%d bytes read\n",ret); 
	}
